...
//  summer composite. The pre-fire and the post-fire        //
//  were set as the same year as the fire.                  //
//  The compositing criterion is the max-min                //
//                                                          //
...
//  GEE link: https://code.earthengine.google.com/e686cf27d8c8b629c46a7418b2c74704
//                                                          //
//  License: MIT License                                    //
...
var get_severity = function(fire) {
  ...
  // Post NBR: Jun 1 - Oct 1 of the Year Of Fire (YOF)
  // This captures the post-fire condition by using a quality mosaic to select the best pixels based on 'nbr_sort'.
  ...
  // Pre NBR: Jun 1 - Oct 1 of the Year Of Fire (YOF)
  // This captures the pre-fire condition by filtering images with dates before the post-fire image's time.
  var preFireCol = lsCol.filterBounds(study_site)
                        .filterDate(Fireyear, Fireyear.advance(1, 'year'))
                        .filter(ee.Filter.dayOfYear(153, 274))
                        .map(function(Img) {
                          var time = Img.select('time');
                          return Img.updateMask(time.lt(postNBR.select('time')));
                        });

  var preNBR = preFireCol.qualityMosaic('nbr');
...
for (var year = year_start; year <= year_end; year++) {
  ...
  Export.image.toDrive({
    ...
    description: 'Summer_max_min_' + year,
    folder: 'YourFolder',
    maxPixels: 1e13,
    scale: 30
  });
